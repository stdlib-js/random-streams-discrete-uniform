...
function RandomStream( a, b, options ) {
	...
	if ( !isInteger( a ) ) {
		throw new TypeError( 'invalid argument. First argument must be a primitive integer. Value: `'+a+'`.' );
	}
	if ( !isInteger( b ) ) {
		throw new TypeError( 'invalid argument. Second argument must be a primitive integer. Value: `'+b+'`.' );
	}
	if ( a > b ) {
