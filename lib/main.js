...
var rduniform = require( '@stdlib/random-base-discrete-uniform' ).factory;
...
function RandomStream( a, b, options ) {
	...
	if ( !isInteger( a ) ) {
		throw new TypeError( format( 'invalid argument. First argument must be a primitive integer. Value: `%s`.', a ) );
	}
	if ( !isInteger( b ) ) {
		throw new TypeError( format( 'invalid argument. Second argument must be a primitive integer. Value: `%s`.', b ) );
	}
	if ( a > b ) {
