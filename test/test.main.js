...
tape( 'the function throws an error if minimum support `a` is greater than maximum support `b`', function test( t ) {
	var values;
	var i;
...
tape( 'attached to the returned stream is the underlying PRNG', function test( t ) {
	...
	t.strictEqual( s.PRNG, minstd, 'has property' );
...
tape( 'the constructor returns a stream for generating pseudorandom numbers from a discrete uniform distribution', function test( t ) {
	var iStream;
	var result;
	var rand;
	var opts;
...
tape( 'the constructor returns a stream for generating pseudorandom numbers from a discrete uniform distribution (object mode)', function test( t ) {
	var iStream;
	var count;
	var rand;
	var opts;
...
tape( 'the constructor supports returning a seeded readable stream', function test( t ) {
	...
	s1 = randomStream( 2, 5, opts );
	s1.on( 'end', onEnd1 );

	s2 = randomStream( 2, 5, opts );
	s2.on( 'end', onEnd2 );
